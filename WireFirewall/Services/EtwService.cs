...
namespace HackerFirewall.Services
{
    public class EtwService : IDisposable
    {
        ...
        private readonly ConcurrentQueue<TrafficEntry> _eventQueue = new ConcurrentQueue<TrafficEntry>();
        private const int MaxQueueSize = 1000;

        public event Action<TrafficEntry> OnNetworkEvent;

        #region Native ETW Structures
        ...
        private unsafe struct EtwEventHeader
        {
            ...
            public fixed byte AddressUnion[32];
        ...
        private static extern uint StartTrace(out long sessionHandle, [MarshalAs(UnmanagedType.LPWStr)] string sessionName, IntPtr properties);

        [DllImport("advapi32.dll", EntryPoint = "ControlTraceW", CharSet = CharSet.Unicode, SetLastError = true)]
        private static extern uint ControlTrace(long sessionHandle, [MarshalAs(UnmanagedType.LPWStr)] string sessionName, IntPtr properties, uint controlCode);

        [DllImport("advapi32.dll", SetLastError = true)]
        private static extern uint EnableTraceEx2(long sessionHandle, ref Guid providerId, uint controlCode, byte level, ulong matchAnyKeyword, ulong matchAllKeyword, uint timeout, IntPtr enableParameters);
        ...
        [DllImport("advapi32.dll", SetLastError = true)]
        private static extern uint ProcessTrace([In] long[] handleArray, uint handleCount, IntPtr startTime, IntPtr endTime);
        ...
        public bool Start()
        {
            ...
            try
            {
                LogService.Log("ETW: Starting...");
                StopExistingSession();

                if (!CreateSession())
                {
                    LogService.Log("ETW: Failed to create session");
                    return false;
                }

                LogService.Log("ETW: Session created");

                if (!EnableProvider())
                {
                    LogService.Log("ETW: Failed to enable provider");
                    StopSession();
                    return false;
                }

                LogService.Log("ETW: Provider enabled");
                ...
                LogService.Log("ETW: Service started successfully");
                return true;
            }
            catch (Exception ex)
            {
                LogService.Log($"ETW: Start failed - {ex.Message}");
                return false;
            }
        }
        ...
        public void Stop()
        {
            ...
            {
                LogService.Log($"ETW: Stop error - {ex.Message}");
            }
            LogService.Log("ETW: Service stopped");
        }
        ...
        private void StopExistingSession()
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        Wnode = new WNODE_HEADER { BufferSize = (uint)bufferSize },
        ...
        private bool CreateSession()
        {
            ...
            try
            {
                ...
                {
                    string desc = result == 5 ? " (ACCESS_DENIED - run as Administrator)" : "";
                    LogService.Log($"ETW: StartTrace error {result}{desc}");
                    return false;
                }
                return true;
            }
            ...
            {
                LogService.Log($"ETW: CreateSession exception - {ex.Message}");
                return false;
            }
            finally { Marshal.FreeHGlobal(buffer); }
        }
        ...
        private bool EnableProvider()
        {
            try
            {
                ...
                uint result = EnableTraceEx2(_sessionHandle, ref guid,
                    EVENT_CONTROL_CODE_ENABLE_PROVIDER, 5, 0xFFFFFFFFFFFFFFFF, 0, 0, IntPtr.Zero);

                if (result != ERROR_SUCCESS)
                {
                    LogService.Log($"ETW: EnableTraceEx2 error {result}");
                    return false;
                }
                return true;
            }
            ...
            {
                LogService.Log($"ETW: EnableProvider exception - {ex.Message}");
                return false;
            }
        }
        ...
        private void ProcessEvents()
        {
            try
            {
                ...
                if (_traceHandle == INVALID_PROCESSTRACE_HANDLE || _traceHandle == 0)
                {
                    ...
                    LogService.Log($"ETW: OpenTrace failed, error {error}");
                    return;
                }

                LogService.Log($"ETW: OpenTrace success, handle={_traceHandle}");
                ...
                if (result != ERROR_SUCCESS && _isRunning)
                    LogService.Log($"ETW: ProcessTrace ended, code {result}");
            }
            catch (Exception ex)
            {
                if (_isRunning)
                    LogService.Log($"ETW: ProcessEvents error - {ex.Message}");
            }
        }
        ...
        private void EventCallback(ref EVENT_RECORD eventRecord)
        {
            try
            {
                if (eventRecord.EventHeader.ProviderId != ProviderGuid) return;
                if (eventRecord.EventHeader.EventDescriptor.Id != ETW_EVENT_ID_NETWORK) return;
                if (eventRecord.UserData == IntPtr.Zero) return;

                int headerSize = Marshal.SizeOf<EtwEventHeader>();
                if (eventRecord.UserDataLength < headerSize) return;
            ...
            {
                LogService.Log($"ETW: Callback error - {ex.Message}");
            }
        }
        ...
        private unsafe TrafficEntry ConvertToTrafficEntry(EtwEventHeader header, byte[] packetData)
        {
            ...
            {
                int length = 0;
                while (length < 64 && header.ProcessName[length] != 0) length++;
                if (length > 0)
                {
                    byte[] nameBytes = new byte[length];
                    for (int i = 0; i < length; i++) nameBytes[i] = header.ProcessName[i];
            ...
            if (header.AddressFamily == ADDRESS_FAMILY_IPV4)
            {
                uint localIpV4 = BitConverter.ToUInt32(new byte[] {
                    ...
                    header.AddressUnion[2], header.AddressUnion[3] }, 0);
                uint remoteIpV4 = BitConverter.ToUInt32(new byte[] {
                    header.AddressUnion[4], header.AddressUnion[5],
                    header.AddressUnion[6], header.AddressUnion[7] }, 0);
        ...
        {
            TrafficEntry dummy;
            while (_eventQueue.TryDequeue(out dummy)) { }
        }
